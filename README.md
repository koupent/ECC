...
<!-- Separate the upstream security warning from the fork-specific note. -->
...
5. An explicit follow-up change request after the Draft PR keeps the same Issue, branch, and PR, but reopens the delivery: the recorded completion, commit, and review evidence are dropped so steps 3 and 4 apply again. A question or review-only follow-up does not reopen it. Until the delivery reopens, only read-only inspection and the recorded reset command are allowed: edits, shell mutations, branch switches, and commits fail closed, and the task cannot stop if the worktree or `HEAD` moved anyway. Read-only inspection is allowlisted per argument, not per binary, so options that run another program — `rg --pre`, `file -C`, `git -c` / `--exec-path` / `--ext-diff` / `--textconv`, `gh --web` — are denied with every unknown option. Only a request naming a different Issue or PR — as `Issue #300` / `PR #300`, as `Pull Request #300`, or as a canonical GitHub `/issues/300` or `/pull/300` URL — starts a new delivery, and the previous record is retained in external state. A named PR is resolved first and binds the new delivery to that PR's head branch and linked Issue; if the PR is not open, comes from a fork, links no Issue, or its head branch is missing locally, preparation fails closed instead of creating an Issue or branch.
...
**That's it!** You now have access to 67 agents, 281 skills, and 99 legacy command shims.
...
| Commands | PASS: 99 commands   | PASS: 35 commands | **Claude Code leads** |
...
| **Commands** | 99                    | Shared | Instruction-based | 35 | 5 prompts |
...
| **Version** | Plugin | Plugin | Reference config | 2.1.0-koute.25 | Instruction layer |
