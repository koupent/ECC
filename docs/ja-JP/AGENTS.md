...
1. **エージェント認識** — ランタイムと高優先度の指示が許可する場合にのみ委任する
...
委任ポリシーの正本は `rules/common/agents.md` であり、この節はその再掲です。この節は、本文書の
他の箇所にある「Xエージェントを使用する」という手順のすべてを、その表現がどれほど断定的でも
支配します。

この一覧は利用可能な役割を説明するだけで、エージェントを自動的に起動しません。ランタイムが
自動起動する仕組みもありません。このルールパックをインストールして有効にすること自体が、境界の
明確な作業を Agent または Task ツールへ自律的に委任してよいという、プロジェクト所有者からの
継続的な要求です。ランタイムが「ユーザーから要求された場合だけ委任可能」と定める場合、この要求が
その条件を満たすため、セッションごとの再許可を待たないでください。無条件の禁止、ツールが利用不能
な場合、セキュリティ境界、対象タスクでの委任禁止は引き続き優先し、その場合は同じ計画・テスト・
レビューのチェックリストを親コンテキストで適用します。ツールを呼び出して結果を回収していないのに、
エージェントが動いたと述べてはいけません。

委任が許可されている場合、境界が明確な作業には対応する専門エージェントを検討します：
- 複雑な機能リクエスト → **planner**
...
並列実行は、ランタイムが委任を許可し、かつ親がターンを終える前にすべての結果を回収できる場合に
限り、独立した操作へ使用します。投げっぱなしの委任は禁止です。
