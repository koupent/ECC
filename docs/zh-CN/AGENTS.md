...
这是一个**生产就绪的 AI 编码插件**，提供 67 个专业代理、281 项技能、101 条命令以及自动化钩子工作流，用于软件开发。

**版本:** 2.1.0-koute.30

## 核心原则

1. **智能体感知** — 仅在运行时与更高优先级指令允许时才委派
...
委派策略的正本是 `rules/common/agents.md`，本节是它的重述，并且管辖本文档其他各处的每一条
「使用 X 智能体」的步骤，无论该步骤的措辞多么绝对。

本目录只是列出可用角色，不会自动启动智能体，运行时也不会自行启动。只有当运行时提供所需工具，且
系统、运行时或 harness、组织、项目、用户的更高优先级指令允许时，才可委派；在该范围内，请自行
判断是否委派，而不必等待用户提出要求。当 harness 限制委派时，请遵循 harness，并在父上下文中套用
同样的规划、测试与审查清单。在没有调用工具并回收结果的情况下，绝不可声称智能体已运行。

当允许委派时，对边界清晰的工作可考虑对应的专业智能体：
...
仅当运行时允许委派，且父模型能在结束本轮之前回收全部结果时，才对独立操作使用并行执行。禁止发出
后不回收结果的委派。
