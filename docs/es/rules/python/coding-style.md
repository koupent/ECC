...
del proyecto. Resuelve el formateador y el ordenador de imports por separado: un
proyecto puede combinar `black` con las reglas de imports de ruff, o
`ruff format` con `isort`.
...
3. Ninguno es dependencia — añade `ruff` y usa `ruff format`, igual que el
   quality gate del propio ECC.
...
3. Ninguno es dependencia — añade `ruff` y usa `ruff check --select I --fix`.
